...
`configs/models.yaml` shall declare providers and models. Each provider entry shall specify `base_url`, a credential reference, rate-limit parameters, and a list of supported model identifiers. The credential reference is `api_key_env` (an environment variable name) for the `dotenv` and `env_var` backends, or `api_key_secret` (a Key Vault secret name) for the `azure_keyvault` backend. A provider entry may specify both fields when it supports multiple backends. The `azure_keyvault` backend strictly requires `api_key_secret`; a provider entry that omits it raises `ConfigValidationError` at load. Each model entry shall specify token-cost rates.
